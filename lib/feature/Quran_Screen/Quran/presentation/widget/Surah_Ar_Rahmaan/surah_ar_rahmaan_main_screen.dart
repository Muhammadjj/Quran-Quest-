...
class QuranSurahArRahmaanScreen extends StatefulWidget {
  const QuranSurahArRahmaanScreen({super.key});

  @override
  State<QuranSurahArRahmaanScreen> createState() =>
      _QuranSurahArRahmaanScreenState();
}

class _QuranSurahArRahmaanScreenState extends State<QuranSurahArRahmaanScreen> {
  @override
  void initState() {
    super.initState();
  ...
  void triggerSurahArRahmaanIndex() {
    context
        ...
        .add(QuranSurahDetailFetchByIndexEvent(surahIndex: 55));
  }
        ...
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            return CustomScrollView(
              slivers: [
                SliverAppBarSurahArRahmaanWidget(
                  width: width,
                ),
                _SurahArRahmaanContent(
                  height: height,
                  width: width,
                ),
              ],
...
class _SurahArRahmaanContent extends StatefulWidget {
  const _SurahArRahmaanContent({required this.height, required this.width});
  final double height;
  final double width;

  @override
  State<_SurahArRahmaanContent> createState() => _SurahArRahmaanContentState();
}

class _SurahArRahmaanContentState extends State<_SurahArRahmaanContent> {
                  ...
                  if (ayahIndex >= 0 &&
                    ...
                    return SurahDetailCardWidget(
                      ...
                      playerAudioPress: () => _audioHandler.toggleAudio(
                        state.detailModel.data.ayahs[index].audio,
                        ayahIndex,
                        setState,
                      ),
                      isPlaying:
